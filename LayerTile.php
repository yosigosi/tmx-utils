...
class LayerTile extends Layer {
  ...
  function load_data() {
    if (strlen ( $this->data ) < $this->map->width * $this->map->height * 4) {
      $this->data = Tools::parse_data ( $this->data, $this->encoding, $this->compression );
    }
  }
  ...
  public function reverse_row() {
    for($i = 0; $i < $this->height; ++ $i) {
      for($j = 0; $j < floor ( $this->width / 2 ); ++ $j) {
        for($a = 0; $a < 4; ++ $a)
          Tools::swap_array_items ( $this->data, ($i * $this->width + $j) * 4 + $a, (($i + 1) * $this->width - 1 - $j) * 4 + $a );
      }
    }
  }
  public function reverse_col() {
    for($j = 0; $j < $this->width; ++ $j) {
      for($i = 0; $i < floor ( $this->height / 2 ); ++ $i) {
        for($a = 0; $a < 4; ++ $a)
          Tools::swap_array_items ( $this->data, ($i * $this->width + $j) * 4 + $a, (($this->height - 1 - $i) * $this->width + $j) * 4 + $a );
      }
    }
  }
