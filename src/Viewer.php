...
class Viewer {
  ...
  // methods
  public function setMap(Map $map, $rot = '') {
    // rotate map if it is specified
    if (! empty ( $rot )) {
      $map = $this->rotateMap ( $rot, $map );
    }
    $this->map = $map;
  }
      ...
      foreach ( $map->layers as $a => $layer ) {
        for($i = 0; $i < $map->height; ++ $i) {
          for($j = 0; $j < $map->width; ++ $j) {
            ...
            // var_dump($lid);
            if ($rot == 'cw' || $rot == '270') {
              $lid = Tools::rotate90cw_lid ( $lid );
            } elseif ($rot == 'ccw' || $rot == '90') {
              $lid = Tools::rotate90cw_lid ( $lid );
            } elseif ($rot == '180') {
              $lid = Tools::rotate180_lid ( $lid );
            }
            // var_dump($lid);
            // echo '<br/>';
            if ($lid == - 1)
              ...
              $tile = $lid + $map->tilesets [$map->get_tileset_index ( $tile )]->firstgid;
            // var_dump($tile);
  ...
  public function load_ts() {
    ...
    foreach ( $this->map->tilesets as $i => $ts ) {
      ...
      if (( bool ) $transc && $transc != '') {
        ...
        Tools::my_transparent ( $this->ts_imgs [$i], $r, $g, $b, $trans );
      ...
      for($a = 0; $a < $tsw - $this->map->tilesets [$i]->margin; $a += $this->map->tilesets [$i]->tilewidth) {
        ++ $this->ts_largeur [$i];
        if ($this->map->tilesets [$i]->spacing > 0)
          $a += $this->map->tilesets [$i]->spacing;
      }
  ...
  private function draw_tile(&$ly, $cgid, $i = NULL, $j = NULL, &$o = NULL) {
    ...
    $tx2 = 0;
    if ($this->map->tilesets [$ti]->spacing > 0)
      $tx2 += $this->map->tilesets [$ti]->spacing * $tx;
    if ($this->map->tilesets [$ti]->margin > 0)
      $tx2 += $this->map->tilesets [$ti]->margin;
    $ty = ( int ) ($lid / $this->ts_largeur [$ti]);
    $ty2 = 0;
    if ($this->map->tilesets [$ti]->spacing > 0)
      $ty2 += $this->map->tilesets [$ti]->spacing * $ty;
    if ($this->map->tilesets [$ti]->margin > 0)
      $ty2 += $this->map->tilesets [$ti]->margin;
    ...
    if ($this->draw_objects) {
      foreach ( $ol->getAllObjects () as $o ) {
        if ($o->polygon || $o->polyline) {
          if ($o->x + $o->getWidthR () <= $x * $this->map->tilewidth)
            continue;
          if ($o->y + $o->getHeightB () <= $y * $this->map->tileheight)
            continue;
          if ($o->x - $o->getWidthL () >= ($x + $w) * $this->map->tilewidth)
            continue;
          if ($o->y - $o->getHeightT () >= ($y + $h) * $this->map->tileheight)
            continue;
          imagerectangle ( $this->img, $this->ox + ($o->x - $o->getWidthL ()) * $this->zoom, $this->oy + ($o->y - $o->getHeightT ()) * $this->zoom, $this->ox + ($o->x + $o->getWidthR ()) * $this->zoom, $this->oy + ($o->y + $o->getHeightB ()) * $this->zoom, $this->colors ['ligra'] );
          if ($o->polyline) {
          ...
          if ($o->x + $o->width <= $x * $this->map->tilewidth)
            continue;
          if ($o->y + $o->height <= $y * $this->map->tileheight)
            continue;
          if ($o->x >= ($x + $w) * $this->map->tilewidth)
            continue;
          if ($o->y >= ($y + $h) * $this->map->tileheight)
            continue;
          imagesetthickness ( $this->img, 2 );
          ...
          // var_dump($o);die();
          // var_dump($o->x);die();
          if ($o->x + $this->map->tilesets [$ti]->tilewidth <= $x * $this->map->tilewidth)
            continue;
          if ($o->y + $this->map->tilesets [$ti]->tileheight <= $y * $this->map->tileheight)
            continue;
          if ($o->x >= ($x + $w) * $this->map->tilewidth)
            continue;
          if ($o->y >= ($y + $h) * $this->map->tileheight)
  ...
  public function draw_layers($x = 0, $y = 0, $w = PHP_INT_MAX, $h = PHP_INT_MAX) {
    ...
    foreach ( $this->map->layers as $index => $ly ) {
      ...
      if ($ly instanceof LayerTile) {
        if ($this->draw_tiles) {
          // break;
          $this->draw_tilelayer ( $ly, $x, $y, $w, $h );
        }
      } elseif ($ly instanceof LayerObject) {
        if ($this->draw_objects) {
          // die();
          $ol = $ly;
          $this->draw_objectlayer ( $ly, $x, $y, $w, $h );
        }
      } elseif ($ly instanceof LayerImage) {
        if ($this->draw_images) {
          $il = $ly;
          $img_ = Tools::create_image_from( dirname ( $this->map->filename ) . '/' . $il->source );
          if ($il->x + imagesx ( $img_ ) <= $x * $this->map->tilewidth) {
            continue;
          }
          if ($il->y + imagesy ( $img_ ) <= $y * $this->map->tilewidth) {
            continue;
          }
          if ($il->x >= ($x + $w) * $this->map->tilewidth) {
            continue;
          }
          if ($il->y >= ($y + $h) * $this->map->tileheight) {
            continue;
          }
          if (function_exists ( 'imageantialias' )) {
          ...
          if (( bool ) $transc && $transc != '') {
            ...
            Tools::my_transparent( $img_, $r, $g, $b, $trans );
          ...
          if ($this->map->orientation == 'orthogonal') {
            ...
            if ($this->zoom == 1) {
              Tools::image_copy_and_resize ( $this->img, $img_, $this->ox + $il->x, $this->oy + $il->y, 0, 0, $sw, $sh );
            } else {
