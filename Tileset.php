...
class Tileset {
  ...
  public static function load_xml($filename) {
    return Tools::load_xml_from_file ( $filename );
  }
  ...
  public function load_from_tsx($filename) {
  ...
  public function load_from_element(\SimpleXMLElement $xml, $recur = true) {
    if (( bool ) $xml ['source'] != FALSE) {
      ...
      if ($recur) {
        $this->load_from_tsx ( dirname ( $this->map->filename ) . '/' . $this->sourceTSX );
      }
      return;
    }
    if (isset ( $xml ['firstgid'] ))
      $this->firstgid = ( int ) $xml ['firstgid'];
    if (( bool ) $xml ['firstgid'] != FALSE)
      $this->firstgid = ( int ) $xml ['firstgid'];
        ...
        if (( bool ) $tile->image != false) {
          ...
          if (strlen ( $this->tiles [( string ) $tile ['id']] ['imagecontent'] ) > 0) {
            if (strlen ( $this->tiles [( string ) $tile ['id']] ['imageencoding'] ) > 0 && strlen ( $this->tiles [( string ) $tile ['id']] ['imagecompression'] ) > 0) {
              $this->tiles [( string ) $tile ['id']] ['imagecontent'] = Tools::parse_data ( $this->tiles [( string ) $tile ['id']] ['imagecontent'], $this->tiles [( string ) $tile ['id']] ['imageencoding'], $this->tiles [( string ) $tile ['id']] ['imagecompression'] );
            }
          }
    ...
    if (( bool ) $xml->tile !== false) {
      $this->loadIdProperties_from_element ( $xml->xpath ( 'tile' ) );
    }
    return $xml;
  }
